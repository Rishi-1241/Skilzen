...
def main():
    ...
    total_start_time = time.time()  # Record the start time of the entire cycle
    ...
    while True:
        try:
            ...
            # Measure the transcription latency
            ...
            logging.info(Fore.YELLOW + f"Transcription latency: {transcription_latency:.2f} seconds" + Fore.RESET)
            ...
            # Measure the response generation latency
            ...
            response_generation_latency = response_end_time - response_start_time
            logging.info(Fore.YELLOW + f"Response generation latency: {response_generation_latency:.2f} seconds" + Fore.RESET)
            ...
            # Calculate the total latency (from user input to audio response)
            total_end_time = time.time()
            total_latency = total_end_time - total_start_time
            logging.info(Fore.GREEN + f"Total latency (from user input to audio response): {total_latency:.2f} seconds" + Fore.RESET)
